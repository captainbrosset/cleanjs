...
var Bookmarks = Module("bookmarks", {
    ...
    get format() ({
        anchored: false,
        title: ["URL", "Info"],
        keys: {text: "url", description: "title", icon: "icon", extra: "extra", tags: "tags", isURI: function () true, folder: "folder" },
    ...
    add: function add(unfiled, title, url, keyword, tags, force) {
        // FIXME
        if (isObject(unfiled))
            var { id, unfiled, title, url, keyword, tags, post, charset, force, folder} = unfiled;
    ...
    commands: function () {
        ...
        const folder = {
            names: ["-folder", "-f"],
            description: "A folder of bookmark",
            completer: function folder(context, args) {
                completion.folder(context, args['-folder'] || "")
            },
        ...
        const title = {
            ...
            completer: function title(context, args) {
                if (!args.bang)
                    return [
                            [tab.label, tab.linkedBrowser.currentURI.spec]
                            for ([,tab] in Iterator(tabs.allTabs))
                        ]
        ...
        commands.add(["bma[rk]"],
            ...
            function (args) {
                dactyl.assert(!args.bang || args["-id"] == null,
                              _("bookmark.bangOrID"));
                let opts = {
                    folder: !args['-folder'] ? null : foldercache.getFolderIdByPath(args["-folder"]) || foldercache.createFolder(args["-folder"]),
                    force: args.bang,
                    unfiled: false,
                    id: args["-id"],
                ...
                completer: function (context, args) {
                    if (!args.bang) {
                        context.title = ["Page URL"];
                            context.completions = [[tabs.getTab().linkedBrowser.currentURI.spec, tabs.getTab().label]];
                        return;
                    }
        ...
        commands.add(["bmarks"],
            ...
            function (args) {
                bookmarks.list(args.join(" "), args["-tags"] || [], args.bang, args["-max"],
                               { keyword: args["-keyword"], title: args["-title"], folder: args["-folder"] });
            },
            {
                bang: true,
                completer: function completer(context, args) {
                    context.filter = args.join(" ");
                    completion.bookmark(context, args["-tags"], { keyword: args["-keyword"], title: args["-title"], folder: args["-folder"] });
                },
                options: [tags, keyword, title, folder,
                    {
                        names: ["-max", "-m"],
        ...
        commands.add(["delbm[arks]"],
            ...
            },
            {
                ...
                literal: 0,
                options: [tags, title, keyword, folder],
                privateData: true
            });
    },
    mappings: function () {
    ...
    options: function () {
        ...
        // Move this option to foldercahce module ?
        options.add(["foldericon", "fic"],
            "The icon to show in folder suggestion menu",
            "string", Folder().icon, {
                completer: function (context) completion.file(context, true),
                setter: function (val) {
                        update(Folder.prototype, {
                            icon: /^\//.test(val) ? "file://" + val : val
                        });
                    },
                // TODO: add chrome scheme
                // Test on windows
                validator: function validator(item) {
                   let match = util.regexp(<![CDATA[
                                   ^ (?P<prefix>
                                       (?P<scheme> chrome|file)
                                       (:\/\/)
                                       )?
                                   (?P<path> .*)
                                   $
                    ...
                    if (!match) 
                        return;
                    let ext = (item.split(".").slice(-1))[0]
                    if (!ext || ["png", "jpg", "ico", "gif"].indexOf(ext) == -1)
                        return;
                    if (!match.scheme || match.scheme === "file")
                        return io.File(item).exists();
                    // For now we don't check if these files exist
                    else if (match.scheme === "chrome")
                        return true;
                    else if (match.scheme === "resource") 
                        return true;
                }
            }
        );
    ...
    completion: function () {
        ...
        completion.folder = function folder (context, folder) {
            context.format = {
                anchored: false,
                keys: {text: "path", path: "path", title: "title", icon: "icon"},
                process: [template.folderIcon, template.folderDescription]
        };
            context.generate = function () values(foldercache.folders);
            completion.folders(context, folder);            
        };
